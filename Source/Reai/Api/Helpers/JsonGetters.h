...
#define IF_JSON_OBJECT_EXISTS(json_obj, name) if (cJSON_GetObjectItemCaseSensitive (json_obj, name))

#define GET_OPTIONAL_JSON_BOOL(json_bool, name, var)                                               \
    IF_JSON_OBJECT_EXISTS (json_bool, name) {                                                      \
        GET_JSON_BOOL (json_bool, name, var);                                                      \
    }                                                                                              \
    else {                                                                                         \
        var = 0;                                                                                   \
    }
...
#define GET_OPTIONAL_JSON_STRING_ARR(json_str_arr, name, var)                                      \
    IF_JSON_OBJECT_EXISTS (json_str_arr, name) {                                                   \
        GET_JSON_STRING_ARR (json_str_arr, name, var);                                             \
    }                                                                                              \
    else {                                                                                         \
        var = 0;                                                                                   \
    }
...
#define GET_JSON_API_ERROR(json_api_error, api_error)                                              \
    do {                                                                                           \
        GET_JSON_STRING (json_api_error, "code", api_error.code);                                  \
        GET_JSON_STRING (json_api_error, "message", api_error.message);                            \
    } while (0)
